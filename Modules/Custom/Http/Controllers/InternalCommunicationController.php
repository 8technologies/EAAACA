...
class InternalCommunicationController extends Controller
{
    ...
    {
        if ($request->user()->cannot('viewAny', InternalCommunication::class)) {
        ...
        if(checkAjaxJsonRequest($request))
        {
            ...
            return Inertia::render('Custom/InternalCommunication/Index', []);
        }
    }
    ...
    {
        if ($request->user()->cannot('create', InternalCommunication::class)) {
        ...
        return Inertia::render('Custom/InternalCommunication/Create', ['data' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $entity = InternalCommunication::create($data);
        $entity->updateRelatedEntities($request);
        $statusText = 'Internal Communication "' . $entity['name'] . '" has been created';

        if(checkAjaxJsonRequest($request))
        {
            return response($entity)->setStatusCode(200, $statusText);
        }
        return redirect()->route('dashboard.internal-communications.index')->with('success', $statusText);
    }
    ...
    {
        $entity = InternalCommunication::with(
            'user', 
            'media',
            // 'tags', 
        )->findOrFail($id);
        ...
        return Inertia::render('Custom/InternalCommunication/Show', ['data' => $entity]);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Inertia::render('Custom/InternalCommunication/Edit', ['data' => $entity]);
    }
    ...
    {
        $entity = InternalCommunication::findOrFail($id);
        ...
        $entity->updateRelatedEntities($request);
        $statusText = 'Internal Communication "' . $entity['name'] . '" has been updated';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $statusText = 'Internal Communication "' . $entity['name'] . '" has been deleted';
        $entity->delete();
