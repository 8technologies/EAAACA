...
class JobAdvertController extends Controller
{
    ...
    {
        if ($request->user()->cannot('viewAny', JobAdvert::class)) {
        ...
        if(checkAjaxJsonRequest($request))
        {
            ...
            return Inertia::render('Custom/JobAdvert/Index', []);
        }
    }
    ...
    {
        if ($request->user()->cannot('create', JobAdvert::class)) {
        ...
        return Inertia::render('Custom/JobAdvert/Create', ['data' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $entity = JobAdvert::create($data);
        $entity->updateRelatedEntities($request);
        $statusText = 'Job Advert "' . $entity['name'] . '" has been created';

        if(checkAjaxJsonRequest($request))
        {
            return response($entity)->setStatusCode(200, $statusText);
        }
        return redirect()->route('dashboard.job-adverts.index')->with('success', $statusText);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Inertia::render('Custom/JobAdvert/Edit', ['data' => $entity]);
    }
    ...
    {
        $entity = JobAdvert::findOrFail($id);
        ...
        $entity->updateRelatedEntities($request);
        $statusText = 'Job Advert "' . $entity['name'] . '" has been updated';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $statusText = 'Job Advert "' . $entity['name'] . '" has been deleted';
        $entity->delete();
