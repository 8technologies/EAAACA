...
class ContentGalleryController extends Controller
{
    ...
    {
        if ($request->user()->cannot('viewAny', ContentGallery::class)) {
        ...
        if(checkAjaxJsonRequest($request))
        {
            ...
            return Inertia::render('Core/Content/Gallery/Index', []);
        }
    }
    ...
    {
        if ($request->user()->cannot('create', ContentGallery::class)) {
        ...
        return Inertia::render('Core/Content/Gallery/Create', ['data' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $entity = ContentGallery::create($data);
        $entity->updateRelatedEntities($request);
        $statusText = 'Gallery "' . $entity['name'] . '" has been created';

        if(checkAjaxJsonRequest($request))
        {
            return response($entity)->setStatusCode(200, $statusText);
        }
        return redirect()->route('dashboard.gallery.index')->with('success', $statusText);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Inertia::render('Core/Content/Gallery/Edit', ['data' => $entity]);
    }
    ...
    {
        $entity = ContentGallery::findOrFail($id);
        ...
        $entity->updateRelatedEntities($request);
        $statusText = 'Gallery "' . $entity['name'] . '" has been updated';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $statusText = 'Gallery "' . $entity['name'] . '" has been deleted';
        $entity->delete();
