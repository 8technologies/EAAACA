...
class ContentTestimonyController extends Controller
{
    /**
    ...
    {
        if ($request->user()->cannot('viewAny', ContentTestimony::class)) {
        ...
        if(checkAjaxJsonRequest($request))
        {
            ...
            return Inertia::render('Core/Content/Testimony/Index', []);
        }
    }
    ...
    {
        if ($request->user()->cannot('create', ContentTestimony::class)) {
        ...
        return Inertia::render('Core/Content/Testimony/Create', ['data' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $entity = ContentTestimony::create($data);
        $entity->updateRelatedEntities($request);
        $statusText = 'Testimony "' . $entity['name'] . '" has been created';

        if(checkAjaxJsonRequest($request))
        {
            return response($entity)->setStatusCode(200, $statusText);
        }
        return redirect()->route('dashboard.testimonies.index')->with('success', $statusText);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Inertia::render('Core/Content/Testimony/Edit', ['data' => $entity]);
    }
    ...
    {
        $entity = ContentTestimony::findOrFail($id);
        ...
        $entity->updateRelatedEntities($request);
        $statusText = 'Testimony "' . $entity['name'] . '" has been updated';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $statusText = 'Testimony "' . $entity['name'] . '" has been deleted';
        $entity->delete();
