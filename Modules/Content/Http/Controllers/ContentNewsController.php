...
class ContentNewsController extends Controller
{
    ...
    {
        if ($request->user()->cannot('viewAny', ContentNews::class)) {
        ...
        if(checkAjaxJsonRequest($request))
        {
            ...
            return Inertia::render('Core/Content/News/Index', []);
        }
    }
    ...
    {
        if ($request->user()->cannot('create', ContentNews::class)) {
        ...
        return Inertia::render('Core/Content/News/Create', ['data' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $entity = ContentNews::create($data);
        $entity->updateRelatedEntities($request);
        $statusText = 'News "' . $entity['name'] . '" has been created';

        if(checkAjaxJsonRequest($request))
        {
            return response($entity)->setStatusCode(200, $statusText);
        }
        return redirect()->route('dashboard.news.index')->with('success', $statusText);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Inertia::render('Core/Content/News/Edit', ['data' => $entity]);
    }
    ...
    {
        $entity = ContentNews::findOrFail($id);
        ...
        $entity->updateRelatedEntities($request);
        $statusText = 'News "' . $entity['name'] . '" has been updated';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $statusText = 'News "' . $entity['name'] . '" has been deleted';
        $entity->delete();
