...
class ContentServiceController extends Controller
{
    ...
    {
        if ($request->user()->cannot('viewAny', ContentService::class)) {
        ...
        if(checkAjaxJsonRequest($request))
        {
            ...
            return Inertia::render('Core/Content/Service/Index', []);
        }
    }
    ...
    public function addContentSection(Request $request, $id)
    {
        ...
        $data['position'] = $contentService->layoutRows->count() + 1;
        $data['field_name'] = 'content_section';
        $data['user_id'] = $request->user()->id;

        $contentService->layoutRows()->create($data);

        if(checkAjaxJsonRequest($request))
        {
            return ContentService::with(
                array_merge(
                    ['user'],
    ...
    {
        if ($request->user()->cannot('create', ContentService::class)) {
        ...
        return Inertia::render('Core/Content/Service/Create', ['data' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $entity = ContentService::create($data);
        $statusText = 'Service "' . $entity['name'] . '" has been created';

        if(checkAjaxJsonRequest($request))
        {
            return response($entity)->setStatusCode(200, $statusText);
        }
        return redirect()->route('dashboard.services.edit', [$entity->id])->with('success', $statusText);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Inertia::render('Core/Content/Service/Edit', ['data' => $entity]);
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $statusText = 'Service "' . $entity['name'] . '" has been updated';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        return redirect()->route('dashboard.services.index')->with('success', $statusText);
    }
}