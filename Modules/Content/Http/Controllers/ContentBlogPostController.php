...
class ContentBlogPostController extends Controller
{
    /**
    ...
    {
        if ($request->user()->cannot('viewAny', ContentBlogPost::class)) {
        ...
        if(checkAjaxJsonRequest($request))
        {
            ...
            return Inertia::render('Core/Content/BlogPost/Index', []);
        }
    }
    ...
    {
        if ($request->user()->cannot('create', ContentBlogPost::class)) {
        ...
        return Inertia::render('Core/Content/BlogPost/Create', ['data' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $entity = ContentBlogPost::create($data);
        $entity->updateRelatedEntities($request);
        $statusText = 'BlogPost "' . $entity['name'] . '" has been created';

        if(checkAjaxJsonRequest($request))
        {
            return response($entity)->setStatusCode(200, $statusText);
        }
        return redirect()->route('dashboard.blog-posts.index')->with('success', $statusText);
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Inertia::render('Core/Content/BlogPost/Edit', ['data' => $entity]);
    }
    ...
    {
        $entity = ContentBlogPost::findOrFail($id);
        ...
        $entity->updateRelatedEntities($request);
        $statusText = 'BlogPost "' . $entity['name'] . '" has been updated';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $statusText = 'BlogPost "' . $entity['name'] . '" has been deleted';
        $entity->delete();
