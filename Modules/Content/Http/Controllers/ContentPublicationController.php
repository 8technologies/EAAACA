...
class ContentPublicationController extends Controller
{
    ...
    {
        if ($request->user()->cannot('viewAny', ContentPublication::class)) {
        ...
        if(checkAjaxJsonRequest($request))
        {
            ...
            return Inertia::render('Core/Content/Publication/Index', []);
        }
    }
    ...
    {
        if ($request->user()->cannot('create', ContentPublication::class)) {
        ...
        return Inertia::render('Core/Content/Publication/Create', ['data' => $data]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $entity = ContentPublication::create($data);
        $entity->updateRelatedEntities($request);
        $statusText = 'Publication "' . $entity['name'] . '" has been created';

        if(checkAjaxJsonRequest($request))
        {
            return response($entity)->setStatusCode(200, $statusText);
        }
        return redirect()->route('dashboard.publications.index')->with('success', $statusText);
    }
    ...
    {
        $entity = ContentPublication::with(
            'user', 
            'media',
            'tags', 
        )->findOrFail($id);
    ...
    public function edit(Request $request, $id)
    {
        ...
        return Inertia::render('Core/Content/Publication/Edit', ['data' => $entity]);
    }
    ...
    {
        $entity = ContentPublication::findOrFail($id);
        ...
        $entity->updateRelatedEntities($request);
        $statusText = 'Publication "' . $entity['name'] . '" has been updated';
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $statusText = 'Publication "' . $entity['name'] . '" has been deleted';
        $entity->delete();
