    ...
    export default {
        ...
        methods: {
            getEntity() {
                ...
                if (this.data && this.data.media && this.data.thumbnail) {
                ...
                if (this.data && this.data.media_image && this.data.media_image.original) {
                    return this.data.media.find(obj => {
                        if (obj.file_url === this.data.media_image.original) {
                            return obj.file_url === this.data.media_image.original
                        }
                    })
                }
                return null
            ...
            getEntityDetachUrl() {
                var parentEntityUrl = this.getParentWithFormField('image_id').data.entity_links.url_view
                return parentEntityUrl + '?_detach_image=' + this.entity.id
            },
        },
        watch: {
            file: function(newVal, oldVal) {
                this.getParentWithFormField('image').form.image = newVal
            },
            file_id: function(newVal, oldVal) {
                this.getParentWithFormField('image_id').form.image_id = newVal
            },
        },
    }
